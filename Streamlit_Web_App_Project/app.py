...
def main():
    ...
    if page == "Homepage":
        """
        ## Building Streamlit Apps with Matplotlib
...
def four_axes():
    ...
    ax[1, 0].set_xticklabels(bar_data["event"], rotation=45)  # Rotate x-axis labels
    ax[1, 0].set_xlabel("event")
    ax[1, 0].set_ylabel("views")

    views = df.groupby("published_year")["views"].sum().reset_index()
    ax[1, 1].plot(views["published_year"], views["views"])
    ax[1, 1].set_xticklabels(views["published_year"], rotation=45)  # Rotate x-axis labels
